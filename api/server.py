...
@app.route('/api/scrape/greek', methods=['GET'])
def scrape_endpoint_greek():
    try:
        matches = scrape_matches_greek()
        return jsonify({"status": "success", "data": matches}), 200
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500


@app.route('/api/scrape/premier', methods=['GET'])
def scrape_endpoint_premier():
    try:
        matches = scrape_matches_premier()
        return jsonify({"status": "success", "data": matches}), 200
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500
